...
var pmModel =require(path.join(__dirname,"..","models","post-med-model.js"));
...
var fullPath = path.join(__dirname,"..","public","uploads",req.body.mpic);
//--------------------------------------------
...
await postMedModel.deleteOne({uid:req.params.uid,medname:req.params.medname}).then((result)=>{
        console.log(result);
        if(result.deletedCount!=0)
        res.json({msg:"Deleted"});
        else
        res.json({msg:"invalid medicine name"});
    })
}

async function doFetchCity(req,resp){
    postMedModel.distinct("city")
    .then((result)=>{
        console.log(result);
        resp.json(result);
    })
...
async function doFetchProvider(req,resp){
    ...
    postMedModel.find({city:req.params.city,medname:req.params.medname})
    .then((result)=>{
        console.log(result.length);
        resp.json(result);
    })
...
module.exports = {createPostMed,fetchMedicine,deleteMedicine,doFetchMedicine,doFetchProvider,doFetchCity};
